...
DEFAULT_APP = "com.xxxxxxx"

CMD_CPU_TPL = "adb shell top -s cpu|grep --line-buffered -E '%s'"
CMD_MEM_TPL = "adb shell dumpsys meminfo %s | grep --line-buffer 'TOTAL' | awk -F ' ' '{print $2}'"
...
def start_stat():
    ...
    while True:
        ...
        # print(mem_line)
        mem = 0
        if mem_line.endswith("\n"):
            mem_line = mem_line[0:-1]
            if mem_line.isdigit():
        ...
        print(STAT_TPL % (cur_time(), str(round(cpu, 2)), str(round(avg_cpu, 2)), str(round(max_cpu, 2))
                          , str(round(mem, 2)), str(round(avg_mem, 2)), str(round(max_mem, 2)), spend_time, stat_count))
        time.sleep(3)
    return
...
def get_spend_time(start_time:float):
